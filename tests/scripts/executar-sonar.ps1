...
function Invoke-SonarScanner {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Falha no SonarScanner durante '$StepName' (exit code $LASTEXITCODE)."
    }
}
...
$scannerPath = (Get-Command "dotnet-sonarscanner" -ErrorAction Stop).Source
...
try {
    ...
    Write-Host "      docker compose --project-directory . --env-file config/.env -f config/docker-compose.tools.yml up sonarqube -d" -ForegroundColor Red
    exit 1
}
Write-Host "      OK" -ForegroundColor Green
...
try {
    ...
    Invoke-SonarScanner -ScannerPath $scannerPath -Arguments $scannerArgs -StepName "begin"
    Write-Host "      OK" -ForegroundColor Green
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Falha no build da solucao (exit code $LASTEXITCODE)."
    }
    Write-Host "      OK" -ForegroundColor Green
    ...
    Invoke-SonarScanner -ScannerPath $scannerPath -Arguments @("end", "/d:sonar.token=$Token") -StepName "end"
    Write-Host "      OK" -ForegroundColor Green
}
finally {
    Pop-Location
